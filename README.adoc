...
==== Rust Acceleration (prebuilt, zero compilation)

Where the platform gem exists, `gem install omnizip` ships a prebuilt
`omnizip-ffi` cdylib and the codec + archive tiers route through it
automatically — no Rust toolchain, no compilation:

* x86_64-linux, aarch64-linux, arm-linux (glibc and musl)
* arm64-darwin, x86_64-darwin
* x64-mingw-ucrt, aarch64-mingw-ucrt, x64-mingw32

Everywhere else — including any platform without a binary — the gem
stays fully functional on its pure-Ruby core (the acceleration is
strictly optional). JRuby and TruffleRuby get acceleration too
whenever a cdylib is available: they bind through the bundled `ffi`
gem (MRI uses stdlib Fiddle). Three escape hatches:

* `OMNIZIP_NO_RUST=1` — force the pure-Ruby core everywhere.
* `OMNIZIP_FFI_DYLIB=/path/to/libomnizip_ffi` — load a specific build.
* `rake rust:build` — build and vendor the cdylib yourself from a
  checkout (developers).

Any Rust-side failure also falls back to the Ruby core mid-operation,
so `auto` mode is never worse than pure Ruby.
