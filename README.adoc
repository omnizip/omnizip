...
* <<deflate64-compression,Deflate64>> - Enhanced Deflate with 64KB dictionary
...
Omnizip provides two APIs to suit different needs:

[[rubyzip-compatibility]]
=== Option A: Rubyzip Compatibility (Drop-in Replacement)

Use Omnizip as a drop-in replacement for the `zip` gem (rubyzip) with zero code changes:

[source,ruby]
----
require 'omnizip/rubyzip_compat'

# Use familiar rubyzip syntax
...
* Migrating from rubyzip to Omnizip
* Maintaining existing codebases
* Need familiar ZIP API

**Features:**

* Complete `Zip::File`, `Zip::Entry`, `Zip::OutputStream`, `Zip::InputStream` compatibility
* All error classes aliased (`Zip::Error`, etc.)
* Same method signatures as rubyzip

[[native-omnizip-api]]
=== Option B: Native Omnizip API (Convenience Methods)

Use Omnizip's streamlined native API for cleaner, more Ruby-idiomatic code:
...
Omnizip.compress_directory('folder/', 'backup.zip')
Omnizip.extract_archive('archive.zip', 'output/')
Omnizip.list_archive('archive.zip')

# Or use detailed control with Omnizip::Zip namespace
Omnizip::Zip::File.create('custom.zip') do |zip|
  zip.add('file.txt') { 'Content' }
end
----
...
* Want simple one-liner operations
* Prefer modern Ruby idioms
* Need advanced compression features

**Features:**

* Simple convenience methods for common tasks
* Native error handling
* Advanced compression options (LZMA2, filters, etc.)
* Clean, readable code

[[both-apis-together]]
=== Using Both APIs Together

Both APIs work seamlessly on the same archives:
...
Zip::File.create('data.zip') do |zip|
  zip.add('config.yml') { 'config data' }
end

# Modify with native API
Omnizip.add_to_archive('data.zip', 'extra.txt', 'source.txt')

# Read with either API
zip_content = Zip::File.open('data.zip') { |z| z.read('config.yml') }
native_content = Omnizip.read_from_archive('data.zip', 'extra.txt')
----

**Examples:**

* link:examples/quick_start_rubyzip.rb[Rubyzip Compatibility Demo]
* link:examples/quick_start_native.rb[Native API Demo]
* link:examples/api_comparison.rb[Side-by-Side Comparison]
...
Deflate64 (Enhanced Deflate) extends standard Deflate with a 64KB sliding window
(versus 32KB in standard Deflate), providing better compression ratios for larger
files while maintaining ZIP format compatibility as compression method 9.
...
This example demonstrates compressing a large file using Deflate64. The 64KB
window provides better compression for files larger than 32KB compared to
standard Deflate.
====
...
==== Deflate64 advantages

Deflate64 provides several benefits over standard Deflate:

* **Larger Dictionary**: 64KB window (2x larger than Deflate's 32KB)
* **Better Compression**: 2-5% better ratios for files > 32KB
* **ZIP Compatible**: Uses compression method 9 in ZIP archives
* **7-Zip Compatible**: Fully compatible with 7-Zip Deflate64
* **Streaming Support**: Handles large files efficiently

==== Performance characteristics

Compared to standard Deflate:

* **Compression Speed**: 10-15% slower (larger dictionary)
* **Decompression Speed**: 5-10% slower
* **Compression Ratio**: 2-5% better for large files
* **Memory Usage**: ~64KB additional for dictionary

==== Use cases

Deflate64 is optimal for:

* Large text files (source code, logs, CSV)
* ZIP archives requiring better compression
* 7-Zip compatibility scenarios
* Files with repetitive patterns spanning > 32KB
...
**📊 link:BASELINE.md[View Complete v1.0 Performance Baseline]**

The v1.0 baseline establishes performance characteristics tested on 10KB data:

* **LZMA Encode:** 0.205s (4.89 ops/s) - 13-15x slower than native ✅ Acceptable
* **LZMA Decode:** 0.027s (37.01 ops/s) - 8-10x slower than native ✅ Good
* **Range Coder:** 0.002s (498.50 ops/s) - 10x slower than native ✅ Excellent
* **BWT:** 8.716s (0.11 ops/s) - 50-60x slower than native ❌ Critical (needs optimization)
