...
Omnizip::Archive.create('backup.7z', format: :seven_zip,
                        algorithm: :lzma, level: 7) do |archive|
...
Omnizip::Archive.create('app.7z', format: :seven_zip,
                        algorithm: :lzma, level: 9,
                        filters: [:bcj_x86]) do |archive|  # For x86 executables
...
Omnizip::Archive.create("app-v1.0.7z", format: :seven_zip,
                        algorithm: :lzma, level: 9,
                        solid: true) do |archive|  # Even better compression
