...
3. Run independent archives in parallel processes (see the
link:../guides/advanced-features/parallel-processing.html[Parallel Processing guide]):
[source,ruby]
----
dirs = %w[data_a data_b]
pids = dirs.map do |dir|
  Process.fork { Omnizip.compress_directory(dir, "#{dir}.7z") }
end
